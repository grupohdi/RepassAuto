...
import { VeiculoDto } from 'src/dto/VeiculoDto';
...
export class MyVehiclesPage implements OnInit {
  ...
  public company: any;
  public vehicles: any[];
  public carregando: string = "Procurando por Veículos cadastrados...";
        ...
        .then(async (result: any) => {
          ...
          if (result) {
            ...
            if (result.length) {
              ...
              this.carregando = "Nenhum veículo cadastrado ainda.";
            }
          }
          if (refresher)
  ...
  async apagar(vehicleId: string) {

    let attemption = '<b>Atenção!</b><br> Para apagar um veículo, ele não pode estar em nenhuma transação.';
    attemption += 'Tem certeza?';

    const alert = await this.alertController.create({
      subHeader: 'RepassAuto - Cadastro de Veículos',
      message: attemption,
  ...
  async confirmDelete(vehicleId: string) {

    this.loaderCtrl.showLoader(`Apagando...`);

    this.veiculoService.delete(vehicleId)
      .then(async (result: any) => {
      ...
      .catch((error: any) => {
        ...
        this.loaderCtrl.hiddenLoader();
        this.alertCtrl.showAlert('RepassAuto - Meus Veículos', `Erro ao apagar o veículo. `);
      });
